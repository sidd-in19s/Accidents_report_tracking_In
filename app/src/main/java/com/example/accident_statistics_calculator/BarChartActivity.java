...
public class BarChartActivity extends AppCompatActivity implements OnChartValueSelectedListener {
    ...
    private int[] counts = new int[states.length];
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        barChart.setOnChartValueSelectedListener(this); } private void updateChart() {
        String text = "";
        for (int i = 0; i < states.length; i++) {
            if (counts[i] > 0) {
                text += states[i] + ": " + counts[i] + "\n";
            }
        }
        ArrayList states_barchart_arr = new ArrayList<>();
        for (int i = 0; i < states.length; i++) {
            states_barchart_arr.add(new BarEntry(i, counts[i]));
        }
        BarDataSet barDataSet = new BarDataSet(states_barchart_arr, "Road Accidents in all States");
        ...
        barChart.getDescription().setText("State Accident Counts Bar Chart");
    ...
    public void onValueSelected(Entry e, Highlight h) {
        ...
        String selectedState = states[index];
        Intent intent = new Intent(this, DistrictBarChart.class);
